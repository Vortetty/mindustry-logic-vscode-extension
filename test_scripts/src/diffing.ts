...
let new_doc_tmp: string[] = old_doc.split(/\n|\r|\r\n/);
...
for (let i = 0; i < diffed.length; i++) {
    if (diffed[i].added) {
        ...
        if (diffed[i+1] && diffed[i+1].added) {
            if (diffed[i+1].value.match(/^[0-9]+\|.+$/gm)) diffprint += `\x1b[33m${diffed[i].value} -> ${diffed[i+1].value}`;
            else                                           diffprint += `\x1b[31m${diffed[i].value}`;
            i++;
        } else {
