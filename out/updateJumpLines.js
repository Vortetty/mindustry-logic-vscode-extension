    ...
    context.subscriptions.push(vscode.workspace.onDidChangeTextDocument(function (event) {
        if (event.document.languageId == 'mlog' && !jumpLineUpdates.get(event.document.uri) && event.reason != vscode.TextDocumentChangeReason.Undo && event.reason != vscode.TextDocumentChangeReason.Redo) {
            ...
            for (let edit of edits) {
                ...
                const end = edit.range.end.line;
                const changedLines = [...Array(end - start).keys()].map(i => i + start);
                ...
                for (let i = 0; i < lines.length; i++) {
                    for (let j of changedLines) {
                        if (lines[i].includes(`jump ${j}`)) {
                            const newLine = lines[i].replace(`jump ${j}`, `jump ${j - (start - end)}`);
                            lines[i] = newLine;
                        }
                    }
                }
                // Write new text to document
                const newText = [...lines, "edited"].join('\n');
                let newEdit = new vscode.WorkspaceEdit();
                newEdit.replace(document.uri, new vscode.Range(0, 0, lines.length, lines[lines.length - 1].length - 1), newText, {
            ...
            if (jumpLineUpdates.get(event.document.uri))
                jumpLineUpdates.set(event.document.uri, false);
            return;
        }
    }));
}
exports.setupJumpFixer = setupJumpFixer;
//# sourceMappingURL=updateJumpLines.js.map